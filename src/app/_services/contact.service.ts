...
export class ContactService {
  ...
  export(type: string): Observable<any> {
    if (type === 'EMAIL') {
      return this.http.post<any>(
        ServerUrl.B1 + '/api/v1/export?type=EMAIL',
        null
      );
    } else {
      return this.http.post<Blob>(
        ServerUrl.B1 + '/api/v1/export?type=DOWNLOAD',
        null,
  ...
  sendEmail(form): Observable<void> {
    return this.http.post<void>(ServerUrl.B1 + '/api/v1/email', {
      all: true,
  ...
  getSupportedCountries(): Observable<CountryModel[]> {
    return this.http.get<CountryModel[]>(
      ServerUrl.B1 + '/api/v1/contact/countries'
    );
  }

  getSupportedBusinessTypes(): Observable<string[]> {
    return this.http.get<string[]>(
      ServerUrl.B1 + '/api/v1/contact/business-types'
    );
  }
}
